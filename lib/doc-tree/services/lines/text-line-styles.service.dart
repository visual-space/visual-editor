...
class TextLineStyleService {
  final EditorState state;

  TextLineStyleService(this.state);
  ...
  TextStyle getLineStyle(EditorStylesM defaultStyles, LineM line) {
    ...
    AttributeM? block;
    _stylesUtils.getBlocksExceptHeader(line.style).forEach((key, value) {
    ...
    textStyle = applyCustomAttributes(textStyle, line.style.attributes);
  ...
  TextStyle applyCustomAttributes(
    ...
    if (state.config.customStyleBuilder == null) {
      return textStyle;
    }
    ...
    attributes.keys.forEach((key) {
      ...
      if (attr != null) {
        // Custom Attribute
        final customAttr = state.config.customStyleBuilder!(
          attr,
        );
