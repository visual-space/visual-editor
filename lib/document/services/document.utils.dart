...
import '../../markers/services/marker.utils.dart';
...
class DocumentUtils {
  ...
  final _markersUtils = MarkersUtils();
  ...
  // TODO Review if we can use markersUtils directly in Doc ctrl.
  void addBaseAndExtentToMarkers(
    StyleM? style,
    int offset,
    OperationM operation,
  ) {
    _markersUtils.addBaseAndExtentToMarkers(style, offset, operation);
  }
  ...
  void _addNewLineBeforeAndAfterVideoEmbed(
    ...
    if (nextOpIsVideo && operation.data is String && (operation.data as String).isNotEmpty && !(operation.data as String).endsWith('\n')) {
      _insertNewLine(deltaRes);
    }

    // Add newline after video embed
    final opIsInsertVideo = operation.isInsert && operation.data is Map && (operation.data as Map).containsKey(VIDEO_EMBED_TYPE);
    // TODO Use aliases
    final nextOpIsLineBreak =
        i + 1 < operations.length && operations[i + 1].isInsert && operations[i + 1].data is String && (operations[i + 1].data as String).startsWith('\n');
